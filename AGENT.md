...
- Zones have a `zoneType`: `'purdue'` (default; styled by `ZONE_LEVELS`) or `'cluster'` (a hypervisor cluster; solid border, styled by `CLUSTER_ZONE_STYLE`, carries a `guestVMs` roster). Zones can be drawn nested inside one another — `renderZoneElement()` gives smaller-area zones a higher `z-index` so nesting always renders correctly regardless of creation order. Zone membership (which nodes/zones are "in" which zone) is purely geometric — a node or zone's center point falling inside another zone's rectangle — there is no explicit parent/child link stored anywhere; see `getContainingZones()`, `countClusterHosts()`.
