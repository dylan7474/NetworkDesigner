...
- Cable rendering has two modes, toggled by `state.cableRouting` (`'orthogonal'` default, or `'straight'`) via the toolbar's routing button — a view preference like `snapToGrid`/`zoom`, not persisted in `buildExportPayload()`. `renderConnections()` branches on it: straight mode draws an SVG `<line>` between the two node centers (`getNodeCenter()`), same as before; orthogonal mode calls `computeOrthogonalRoute()` to build a 3-segment right-angle `<path>` instead (bends on whichever axis — x or y — has the larger gap, so it reads as one elbow rather than a zigzag). Both the visible cable and its wider invisible click-hitzone use whichever shape tag (`line`/`path`) the mode calls for. The cable badge and VLAN chips anchor on `computeOrthogonalRoute()`'s returned midpoint (the middle segment's own midpoint) in orthogonal mode, not the straight-line endpoint average — for an offset elbow that average wouldn't actually sit on the path.
