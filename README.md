...
- **Firewall Rule Matrix** — generates a table of every cable that crosses a zone boundary (source/dest zone and device, inferred service, VLANs), and flags any rule that links an OT zone (Level 0-3) directly to an IT zone (Level 4-5) without passing through the Industrial DMZ (3.5) — the Purdue model's core security requirement. Exportable to CSV. This is a design-intent reference for documentation and review, not a deployable device configuration.
...
6. Use **Firewall Rules** in the toolbar to generate a cross-zone rule matrix from the current design, review any flagged OT/IT boundary violations, and export it to CSV.
7. Use **Save JSON** / **Load** in the header to export or reimport a complete design, or **Copy Share Link** to hand someone a one-time snapshot of the current design without a file.
